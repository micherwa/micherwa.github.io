        ...
        <BlogContent :useCatalog="true">
            <div slot="content">
                ...
                <p>
                    ...
                    我们共同探讨了这些问题，并最终整理成文，希望帮到有需要的同学。
                </p>

                <h2>事件流的概念</h2>

                <p>
                    先从概念说起，DOM 事件流：将事件分为三个阶段：<code>捕获阶段</code>、<code>目标阶段</code>、<code>冒泡阶段</code>。先调用捕获阶段的处理函数，其次调用目标阶段的处理函数，最后调用冒泡阶段的处理函数。
                    <img src="~@/assets/blog/bg-20170821-01.png">
                </p>

                <p>
                    网景公司提出了 <code>事件捕获</code>的事件流。这就好比矿工的小游戏，每次都会从最上方抛出抓斗，一路往下，捕获矿石。在上图中就是，某个 div 元素触发了某个事件，最先得到通知的是 window，然后是 document，依次往下，直到真正触发事件的那个目标元素 div 为止。
                </p>

                <p>
                    而 <code>事件冒泡</code> 则是由微软提出的，与之顺序相反。可以这样形象地比喻冒泡：一粒泡腾片落入水杯中，泡泡立马从水杯底部往上冒出。在上图中就是，事件会从目标元素 div 开始起泡，再依次往上，直到 window 对象为止。
                </p>
                ...
                <p>
                    ...
                    虽然外层 div 的事件注册在后面，但会先出发。所以，结果是先弹出 <code>outer capture is ok</code>，再弹出 <code>inner capture is ok</code>。
                </p>
                ...
                <h4>5.阻止事件冒泡</h4>
                <p>
                    通常情况下，我们都希望点击某个 div 时，就只触发自己的事件回调。比如，明明点击的是内层 div，但是外层 div 的事件也触发了，这是就不是我们想要的了。这时，就需要阻止事件的冒泡。
                </p>
                <p>
                    事件触发时，会默认传入一个 event 对象，这个 event 对象上有一个方法：<code>stopPropagation</code>。通过此方法，可以阻止冒泡，让外层 div 接收不到事件，自然也就不会触发了。
                ...
                <h2>一些常用技巧</h2>
                <p>
                    回到文章开头的问题：了解事件流的顺序，对日常的工作有什么帮助呢？有以下几点需要注意。
                </p>
                ...
                <p>
                    前面说过 stopPropagation 会阻止事件冒泡。但其实，它更完整的定义是：终止事件在传播过程的捕获、目标处理或起泡阶段进一步传播。事件不再被分派到其他节点上。
                    ...
                    但是，stopPropagation 只会阻止当前元素的事件冒泡或捕获的传播，并不会阻止该元素上其他 click 事件的监听。
                    ...
                    如果想禁用之后所有的 click 事件，就要用到 stopImmediatePropagation 了。但是，需要注意的是，stopImmediatePropagation 只会禁用之后注册的同类型的监听事件。就比如阻止了之后的 click 事件监听函数，但是像 mousedown 之类别的事件，还是可以监听到的。
                ...
                <p>
                    所以，在理解了事件流的概念，清楚了事件捕获与冒泡的顺序，掌握了一些原生事件的技巧之后，相信大家在遇到坑时，可以少走一些弯路了。
                </p>
            </div>
        </BlogContent>
    </div>

<template>
    <div class="blog-container">
        <BlogHeader
            title="async/await" subTitle="一种更优雅的异步写法"
        ...
        <BlogContent :useCatalog="true">
            <div slot="content">
                ...
                <h2>async 函数是什么？</h2>
                <p>
                    <strong>一句话解释：async 函数就是 Generator 函数的语法糖。</strong>
                </p>
                <p>
                    其特点有：
                    <ul>
                        <li>
                            它是建立在promise之上的。所以，不能把它和回调函数搭配使用。但它会隐式地返回一个Promise，因此可以直接return变量，无需使用Promise.resolve进行转换
                        </li>
                        <li>
                            它和promise一样，是非阻塞的。不用写then及其回调函数，减少代码行数，也避免了代码嵌套。所有异步调用可以写在同一个代码块中，无需定义多余的中间变量
                        </li>
                        <li>
                            它可以使异步代码在形式上更接近于同步代码。这就是它最大的价值。
                        </li>
                        <li>
                            它总是与await一起使用的，await 只能在 async 函数体内。
                        </li>
                    </ul>
                </p>
                ...
                <p>
                    ...
                    <pre class="hljs javascript"><code class=""><span class="hljs-function"><span class="hljs-keyword">function</span> <span class="hljs-title">doIt</span>(<span class="hljs-params"></span>) </span>{<br>    <span class="hljs-built_in">console</span>.time(<span class="hljs-string">"doIt"</span>);<br>    <span class="hljs-keyword">const</span> time1 = <span class="hljs-number">300</span>;<br>    step1(time1)<br>        .then(<span class="hljs-function"><span class="hljs-params">time2</span> =&gt;</span> {<br>            <span class="hljs-keyword">return</span> step2(time1, time2)<br>                .then(<span class="hljs-function"><span class="hljs-params">time3</span> =&gt;</span> [time1, time2, time3]);<br>        })<br>        .then(<span class="hljs-function"><span class="hljs-params">times</span> =&gt;</span> {<br>            <span class="hljs-keyword">const</span> [time1, time2, time3] = times;<br>            <span class="hljs-keyword">return</span> step3(time1, time2, time3);<br>        })<br>        .then(<span class="hljs-function"><span class="hljs-params">result</span> =&gt;</span> {<br>            <span class="hljs-built_in">console</span>.log(<span class="hljs-string">`result is <span class="hljs-subst">${result}</span>`</span>);<br>            <span class="hljs-built_in">console</span>.timeEnd(<span class="hljs-string">"doIt"</span>);<br>        });<br>}<br>doIt();</code></pre>
                    用 async/await 来写：
                    <pre class="hljs javascript"><code class=""><span class="hljs-keyword">async</span> <span class="hljs-function"><span class="hljs-keyword">function</span> <span class="hljs-title">doIt</span>(<span class="hljs-params"></span>) </span>{<br>    <span class="hljs-built_in">console</span>.time(<span class="hljs-string">"doIt"</span>);<br>    <span class="hljs-keyword">const</span> time1 = <span class="hljs-number">300</span>;<br>    <span class="hljs-keyword">const</span> time2 = <span class="hljs-keyword">await</span> step1(time1);<br>    <span class="hljs-keyword">const</span> time3 = <span class="hljs-keyword">await</span> step2(time1, time2);<br>    <span class="hljs-keyword">const</span> result = <span class="hljs-keyword">await</span> step3(time1, time2, time3);<br>    <span class="hljs-built_in">console</span>.log(<span class="hljs-string">`result is <span class="hljs-subst">${result}</span>`</span>);<br>    <span class="hljs-built_in">console</span>.timeEnd(<span class="hljs-string">"doIt"</span>);<br>}<br>doIt();</code></pre>
                    更加优雅地实现了。
                </p>
                ...
                <p>
                    因为没有代码块，所以不能在一个返回的箭头函数中设置断点。如果你在一个 .then 代码块中使用调试器的步进(step-over)功能，调试器并不会进入后续的 .then 代码块，因为调试器只能跟踪同步代码的 每一步。
                ...
                <h2>总结</h2>
                <p>
                    JavaScript的异步编写方式，从 回调函数 到 Promise 再到 Async/Await。表面上只是写法的变化，但本质上则是语言层的一次次抽象。让我们可以用更简单的方式实现同样的功能，而不需要去考虑代码是如何执行的。换句话说就是：<strong>异步编程的最高境界，就是根本不用关心它是不是异步</strong>。
                </p>
